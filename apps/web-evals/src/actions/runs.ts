...
export async function createRun({ suite, exercises = [], systemPrompt, ...values }: CreateRun) {
	...
	try {
		...
		const cliCommand = `pnpm --filter @dorjear-code/evals cli --runId ${run.id}`
		...
		const logStream = fs.createWriteStream("/tmp/dorjear-code-evals.log", { flags: "a" })
