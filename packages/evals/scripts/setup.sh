...
build_extension() {
  echo "🔨 Building the Dorjear Code extension..."
  pnpm -w vsix -- --out ../bin/dorjear-code-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/dorjear-code-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  ...
  echo "The Dorjear Code evals system can also be run with Docker on any platform."
  echo "See https://github.com/RooCodeInc/dorjear-code/blob/main/packages/evals/README.md for instructions."
  exit 1
fi
...
if ! code --list-extensions 2>/dev/null | grep -q "RooVeterinaryInc.dorjear-cline"; then
  code --install-extension RooVeterinaryInc.dorjear-cline &>/dev/null || exit 1
fi
...
if [[ ! -d "../../../evals" ]]; then
  ...
  git clone https://github.com/RooCodeInc/dorjear-code-Evals.git ../../../evals || exit 1
  echo "✅ Done"
else
...
echo -n "🗄️ Syncing Dorjear Code evals database... "
pnpm --filter @dorjear-code/evals db:push --force &>/dev/null || exit 1
echo "✅ Done"
...
read -p "💻 Do you want to build a new version of the Dorjear Code extension? [currently $current_version] (y/N): " build_extension
...
if ! nc -z localhost 3000; then
  ...
  if [[ "$start_evals" =~ ^[Yy]|^$ ]]; then
    pnpm --filter @dorjear-code/web-evals dev
  else
    echo "💡 You can start it anytime with 'pnpm --filter @dorjear-code/web-evals dev'."
  fi
else
